...
class SuricataRuleSearcher:
    ...
    def _load_file(self) -> List[str]:
        try:
            ...
            self.logger.error(
                f"File not found: {self.input_file}. Perhaps misspelled? Exiting the program."
            )
            sys.exit(1)
        except IOError:
            self.logger.error(
                f"I/O error occured when reading {self.input_file}. Exiting the program"
            )
            sys.exit(1)
    ...
    def process_file_in_chunks(self) -> None:
        ...
        self.logger.info(
            f"Starting search for service '{self.service_name}' in file '{self.input_file}' with {self.num_threads} threads..."
        )
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for chunk in chunks:
                if chunk:
                    futures.append(
                        executor.submit(self._search_for_service_in_chunk, chunk)
                    )
            ...
            self.logger.info(
                f"Total matches {total_matches} for the service name {self.service_name}"
            )
